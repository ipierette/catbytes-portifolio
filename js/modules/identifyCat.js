...
export function initIdentifyCat() {
  ...
  const showResult = (json) => {
    ...
    const ageBucket = json?.age?.bucket ?? '--';
    const ageYears = json?.age?.yearsApprox ?? null;
    if (spanAge) spanAge.textContent = ageYears != null ? `${ageBucket} (~${ageYears} anos)` : ageBucket;

    const breeds = Array.isArray(json?.breedCandidates) ? json.breedCandidates.slice(0,3) : [];
    if (spanBreed) spanBreed.textContent = breeds.length
      ? breeds.map(b => `${b.label} (${Math.round((b.confidence||0)*100)}%)`).join(', ')
      : '--';

    const cues = Array.isArray(json?.personalityCues) ? json.personalityCues.slice(0,5) : [];
    if (spanPersonality) spanPersonality.textContent = cues.length
      ? cues.map(c => `${c.trait} (${Math.round((c.confidence||0)*100)}%)`).join(', ')
      : '--';
  };
  ...
  form?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (json?.error || (!json.age && !json.breedCandidates && !json.personalityCues)) {
